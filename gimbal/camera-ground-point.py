...
def print_camera_ground_point_example():
    # Example Usage:
...
def camera_ground_point_mils(
    lat, lon,
    heading_mrad,
    gimbal_yaw_mrad, gimbal_pitch_mrad, gimbal_roll_mrad,
    altitude_m
    ):
    """
    Calculates the ground point (latitude, longitude) the camera is looking at.
    ...
    Parameters:
        lat (float): Aircraft latitude in degrees
        lon (float): Aircraft longitude in degrees
        heading_mrad (float): Aircraft heading in milliradians (N=0, E=~1570)
        gimbal_yaw_mrad (float): Gimbal yaw relative to aircraft nose, in milliradians (right = +)
        gimbal_pitch_mrad (float): Gimbal pitch (down from horizontal = +), in milliradians
        gimbal_roll_mrad (float): Gimbal roll (unused here), in milliradians
        altitude_m (float): Altitude of the camera above ground in meters (REQUIRED)
    
    Returns:
        (lat, lon): Tuple of latitude and longitude of ground intersection
    """
    # Convert all angles from milliradians to radians
    heading = heading_mrad / 1000.0
    yaw = gimbal_yaw_mrad / 1000.0
    pitch = gimbal_pitch_mrad / 1000.0
    # Roll is currently unused in this estimation
    # roll = gimbal_roll_mrad / 1000.0
    ...
    ground_dist = altitude_m * math.tan(math.pi / 2 + pitch)  # pitch = -pi/2 (down) => 0 distance (nadir)
...
def print_ground_point_mils_example():
    # Example Usage:
    # Aircraft at (37.0, -122.0), heading 785 mrad (~45 deg), gimbal looking right (1570 mrad ~90 deg),  -785 mrad (~-45 deg) down, altitude 120m
    target = camera_ground_point_mils(
        37.0, -122.0,
        heading_mrad=785,
        gimbal_yaw_mrad=1570,
        gimbal_pitch_mrad=-785,
        gimbal_roll_mrad=0,
        altitude_m=120
    )    
    print(target)
